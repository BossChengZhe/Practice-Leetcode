...
&emsp;&emsp;由于题目中给到的单词将字符串分成两个部分进行扰乱，我们就可以采用递归的方式
* **递归法**：题目中用二叉树的方式分解字符串，并在非叶子节点交换兄弟节点，组成了一个扰乱字符串，重要的只是分割点的选取，分为两种情况
    * 第一种：S1 切割为两部分，然后进行若干步切割交换，最后判断两个子树分别是否能变成 S2 的两部分。
    <div align=center>
    <img src="../Image/87.png">
    </div>

    * 第二种：S1 切割并且交换为两部分，然后进行若干步切割交换，最后判断两个子树是否能变成 S2 的两部分。
    <div align=center>
    <img src="../Image/87.2.png">
    </div>

* **动态规划**：`dp[len][i][j]`表示`s1`,`s2`分别以`i`,`j`开头长度为`len`的子串是否构成扰乱字符串，与上文一样分为两种情况，
    * `s1`的左半部分对应`s2`的左半部分
    $$
    dp_{len,i,j} = dp_{k,i,j}\&\&dp_{len-k,i+k,j+k}    
    $$
    * `s1`的左半部分对应`s2`的右半部分 
    $$
...
&emsp;&emsp;涉及到回文子串的题目下意识使用动态规划碰一碰，本题中`dp[i][j]`表示字符串中以`i`为开头,以`j`为结尾的子串是否是回文子串，状态转移方程是 
$$
dp_{i,j} = s_i==s_j \&\& (j == i +1 || dp_{i+1, j-1})
$$
初始状态为主对角线方向上的所有元素即`j==i`所有单个字符全部都是回文串，从大到小从右下角到左上角动态规划，得到每一个子串是不是回文子串以及回文子串的长度以及个数。